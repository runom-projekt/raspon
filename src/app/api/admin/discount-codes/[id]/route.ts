...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const code = await prisma.$transaction(async (tx) => {
    const current = await tx.discountCode.findUniqueOrThrow({
      where: { id },
      select: { active: true },
    });
    const updated = await tx.discountCode.update({ where: { id }, data: parsed.data });
    await appendAuditLog(tx, {
      actor: session,
      requestId: req.headers.get("x-request-id"),
      action: "DISCOUNT_CODE_STATUS_CHANGED",
